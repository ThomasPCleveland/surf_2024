...
PSElectrode::PSElectrode(G4MaterialPropertiesTable* prop)
{
    //initialize material properties table, verboseLevel, PSKaplanQP,
    ...
    kaplanQP = new PSKaplanQP(matTab);
    ...
    //could also comment out where it is written to file in PSSteppingAction.cc
    reflData = new std::string();
}

PSElectrode::~PSElectrode()
{
    delete kaplanQP;
}

G4bool PSElectrode::IsNearElectrode(const G4Step& /*step*/) const {
...
AbsorbAtElectrode(const G4Track& track, const G4Step& step, G4ParticleChange& particleChange) const
{
    ...
    std::vector<PSPhonon> reflPhonons;
    ...
    PSPhonon primary = PSPhonon(phonEnergy, 0);
    ...
    //could include polarization of phonons as part of physics in PSKaplanQP, not currently accounted for.
    ...
    G4double ERefl = 0.0;
    for (PSPhonon reflPhonon : reflPhonons) {
...
G4ThreeVector PSElectrode::SpecularReflection(G4ThreeVector primaryPhononDir, G4ThreeVector surfNorm) const
{
