...
PSKaplanQP::PSKaplanQP(G4MaterialPropertiesTable* prop, G4int vb)
...
void PSKaplanQP::SetFilmProperties(G4MaterialPropertiesTable* prop) {
    if (!prop) {
        G4Exception("PSKaplanQP::SetFilmProperties()", "G4CMP001",
    ...
    if (!(prop->ConstPropertyExists("gapEnergy") &&
        ...
        G4Exception("PSKaplanQP::SetFilmProperties()", "G4CMP002",
            RunMustBeAborted,
...
G4double PSKaplanQP::
AbsorbPhonon(PSPhonon primary, std::vector<PSPhonon>& reflectedEnergies) const {
    ...
    //errors/warnings
    if (!filmProperties) {
        G4Exception("PSKaplanQP::AbsorbPhonon()", "G4CMP001",
            RunMustBeAborted, "Null MaterialPropertiesTable vector.");
    }

    if (verboseLevel)
        G4cout << "PSKaplanQP::AbsorbPhonon " << primary.GetEnergy() << G4endl;

    if (reflectedEnergies.size() > 0) {
        G4Exception("PSKaplanQP::AbsorbPhonon", "G4CMP007", JustWarning,
    ...
    G4double EDep = 0.;
    std::vector<PSQP> qpEnergies;
    std::vector<PSPhonon> phonEnergies{ primary };
    ...
    G4double ERefl = 0.0;
    for (PSPhonon phonon : reflectedEnergies)
    {
    ...
    if (fabs(energy - ERefl - EDep) / energy > 1e-5) {
        G4cerr << "WARNING PSKaplanQP missing " << (energy - ERefl - EDep) / eV
            << " eV" << G4endl;
    }
...
G4double PSKaplanQP::CalcEscapeProbability(PSPhonon phonon,
    G4double thicknessFrac) const {
    G4double energy = phonon.GetEnergy();
    if (verboseLevel > 1) {
        G4cout << "PSKaplanQP::CalcEscapeProbability E " << energy
...
G4double
PSKaplanQP::CalcQPEnergies(std::vector<PSPhonon>& phonEnergies,
    std::vector<PSQP>& qpEnergies) const {
    if (verboseLevel > 1) {
        G4cout << "PSKaplanQP::CalcQPEnergies QPcut " << lowQPLimit * gapEnergy
            << G4endl;
    }

    // Phonons above the bandgap give all of its energy to the qp pair it breaks.
    G4double EDep = 0.;
    std::vector<PSPhonon> newPhonEnergies;

    //loop through every phonon
    for (PSPhonon phonon : phonEnergies) {
        ...
        //EDep += CalcQPAbsorption(PSQP(E - qpE), newPhonEnergies, qpEnergies);
        qpEnergies.push_back(qp);

        //energy of second QP gets remainder of phonon energy (Eph - Eqp)
        PSQP qp1(E - qp.GetEnergy(), qp.GetTime(), qpLifetime);
        qp1.SetId(qpId);
        //increment QP id
        qpId++;
...
G4double
PSKaplanQP::CalcPhononEnergies(std::vector<PSPhonon>& phonEnergies,
    std::vector<PSQP>& qpEnergies) const {
    if (verboseLevel > 1) {
        G4cout << "PSKaplanQP::CalcPhononEnergies 2*gap " << 2. * gapEnergy
            << " QPcut " << lowQPLimit * gapEnergy << G4endl;
    }

    G4double EDep = 0.;
    G4double lifetime;
    std::vector<PSQP> newQPEnergies;
    ...
    for (PSQP qp : qpEnergies) {
        ...
        if (E >= lowQPLimit * gapEnergy)
        {
            ...
            PSPhonon phon = PhononEnergyRand(qp);
            ...
            //else if (!qp.GetAtGap())
        {
            qp.IncrementTime(0.0);
            //qp.IncrementTime(qpLifetime);
            PSPhonon phon = PSPhonon(E - gapEnergy, qp.GetTime(), zmin);
            phonEnergies.push_back(phon);
        ...
        else //qp is at delta
        {
            ...
            if (qp.GetId() % 2 == 0)
            {
                ...
                PSPhonon phon = PSPhonon(2 * gapEnergy, qp.GetTime(), zmin);
...
void PSKaplanQP::
CalcReflectedPhononEnergies(std::vector<PSPhonon>& phonEnergies,
    std::vector<PSPhonon>& reflectedEnergies) const {
    if (verboseLevel > 1)
        G4cout << "PSKaplanQP::CalcReflectedPhononEnergies " << G4endl;
...
G4double
PSKaplanQP::CalcSubgapAbsorption(PSPhonon phonon,
...
//Not used anymore
G4double
PSKaplanQP::CalcQPAbsorption(PSQP qp,
    ...
    if (qpE >= lowQPLimit * gapEnergy) {
        ...
        EDep += CalcSubgapAbsorption(PSPhonon(qpE - gapEnergy), phonEnergies);
        EDep += gapEnergy;
    }
    else {
        EDep += qpE;
...
PSQP PSKaplanQP::QPEnergyRand(PSPhonon phonon) const {
    ...
    return PSQP(xtest, phonon.GetTime(), qpLifetime);
}

G4double PSKaplanQP::QPEnergyPDF(G4double E, G4double x) const {
...
PSPhonon PSKaplanQP::PhononEnergyRand(PSQP qp) const {
    // PDF is not integrable, so we can't do an inverse transform sampling.
    // Instead, we'll do a rejection method.
    //
    ...
    return PSPhonon(Energy - xtest, qp.GetTime(), zmin);
}

G4double PSKaplanQP::PhononEnergyPDF(G4double E, G4double x) const {
    const G4double gapsq = gapEnergy * gapEnergy;
